...
def information_edit():
        ...
        if image and image.filename != "":
            ...
            cursor.execute("""
                    ...
                    SET username=%s,
                        ...
                        profile_pic=%s
                    WHERE id=%s
            """, (username, email, path, session["user_id"]))
    ...
    return render_template('infomationedit.html')
...
def developer_hub():
    if "user_id" not in session:
        return redirect(url_for("interface"))
    return render_template('Developer Hub.html')
...
def mainpage():
    ...
    return render_template('Mainpage.html', items=processed_items, active_reports=active_reports, found_items=found_items, user=user)
...
def report():
    ...
    """, (session["user_id"], item_type, item_name, location, date, description, phone, image_path))
